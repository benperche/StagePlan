...
export class Renderer {
    constructor() {
        Object.defineProperty(this, "hitTargets", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: []
    ...
    // Semicircle layout
    // ---------------------------------------------------------------------------
    renderSemicircle(ctx, config, w, h) {
        const ox = w / 2;
    ...
    renderArcRow(ctx, row, rowIndex, r, ox, oy, yDir, config, seatNumber) {
        ...
        row.chairs.forEach((chair, chairIndex) => {
            ...
            if (chair.hasStand)
                this.drawStand(ctx, cx, cy, ox, oy);
    ...
    renderStraightRowInArc(ctx, row, rowIndex, r, ox, oy, yDir, config, seatNumber) {
        const enabledChairs = row.chairs.filter(c => c.enabled);
        const total = enabledChairs.length;
        if (total === 0)
            return seatNumber;
        ...
        row.chairs.forEach((chair, chairIndex) => {
            ...
            if (chair.hasStand)
                this.drawStand(ctx, cx, rowY, cx, oy);
            if (config.showNumbers) {
                const num = config.numberRestartPerRow ? enabledIdx + 1 : seatNumber;
        ...
        if (config.showRowLabels)
            this.drawRowLabel(ctx, row.label, startX - CHAIR_HALF - 10, rowY);
        return seatNumber;
    }
    // ---------------------------------------------------------------------------
    // Pure straight layout
    // ---------------------------------------------------------------------------
    renderStraight(ctx, config, w, h) {
        const ox = w / 2;
        const numRows = config.rows.length;
        const oy = this.computeOy(h, numRows, config.flipped);
        const yDir = config.flipped ? 1 : -1;
    ...
    // Drawing primitives
    // ---------------------------------------------------------------------------
    drawChair(ctx, chair, cx, cy, condX, condY, fontSize) {
    ...
    drawStand(ctx, cx, cy, condX, condY) {
        const dx = condX - cx;
        const dy = condY - cy;
    ...
    drawSeatNumber(ctx, cx, cy, condX, condY, num, fontSize) {
        const dx = condX - cx;
        const dy = condY - cy;
        const len = Math.sqrt(dx * dx + dy * dy);
        if (len === 0)
    ...
    drawConductor(ctx, ox, oy, yDir, config) {
        const pw = 52, ph = 36;
        ctx.save();
        ctx.fillStyle = '#555';
