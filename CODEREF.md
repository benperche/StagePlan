...
Every "Move to row" list ends with **+ New** (`appendEmptyRow`), which adds an empty row at the back and moves the selection into it — splitting a crowded row, or starting the next row back from chairs already labelled. It bumps `config.straightRows` when non-zero, because that count is measured from the BACK: appending without it would silently flip the front-most straight row into an arc row. New rows are named by `nextRowLabel()`, which continues A, B, C… while the chart is lettered (as every preset is) and otherwise falls back to the numeric position; "Add row" and the row-count input use it too.

Where a moved block lands: with no explicit slot, `sectionInsertIndex` drops it **after the last chair of the same section** in the destination (matched on `group` or the exact label — deliberately *not* `normalizeSectionLabel`, which strips the part number and would conflate "Vln 1" with "Vln 2"), falling back to appending. Passing an explicit `insertIndex` overrides that, and also makes a **same-row** move meaningful (a reorder); the index is shifted left by however many removed chairs sat before it.

**Bulk chair menu** (`openBulkChairMenu`): marquee-dragging with **no tool armed** opens the single-chair menu's actions for the whole box in one undo step (visibility / seat / stand / colour / labels / move to row). Segment options highlight as `active` only when every selected chair agrees, so a mixed selection honestly shows none. This is the noun-first counterpart to arming a tool and dragging — previously a select-mode drag did nothing at all.

Selecting a chair (a plain click in select mode, a right-click, or a long-press) sets the selection to that one chair as well as opening its menu, so the drop slots appear for a single chair exactly as they do for a marquee — the distinction was arbitrary from the user's side.

**Drop slots** (`selectedChairs` in main.ts; `showDropPoints` / `drawDropPoints` / `dropPointHitTest` in the renderer): the marquee selection *persists* after the bulk menu closes, and while it does, every gap in every row is drawn as an orange dot; clicking one drops the selection at that exact index. Slot positions are derived from the frame's chair `hitTargets` — midpoints between neighbours, extrapolated half a seat past each end (or, for a one-chair row, along the tangent perpendicular to its radius) — so arc and straight rows work without duplicating either row renderer's geometry. Gaps buried inside a contiguous selected run are skipped, since dropping there is a no-op. Selected chairs get a matching orange ring. Cleared by Escape, a click on empty canvas, a completed drop, or a tab change.

**Escape ordering gotcha:** `context-menu.ts` closes itself from a **capture-phase** `keydown` listener, so by the time main.ts's bubble-phase handler runs the menu is already gone and `contextMenuOpen()` reads false. `escapeClosedMenu` — set by a capture listener registered in `bindEvents` *before* any menu can exist, so it runs first — is how the handler still knows that keypress was spent closing the menu. Without it, one Escape would both close the menu and silently drop the selection behind it.
...
- Types: drumkit, piano, guitar-amp, bass-amp, timpani (2–6 drums via `count`), mallet, harp, microphone, gong, cymbal, snare, bass-drum, trap-table, chair, stand, stool, square, rectangle (generic placeholders).
- **Adding an instrument type touches six places** (TS catches four of them — `INSTRUMENT_LABEL` is an exhaustive `Record` and `instrumentLabel` is a returning switch): the `InstrumentType` union (types.ts), a `draw*` glyph (instrument-glyphs.ts), the `drawGlyph` switch + `instrumentLabel` default + `keepUpright` if the glyph is orientation-neutral (renderer.ts), `INSERT_DEFAULTS` (state.ts), `INSTRUMENT_LABEL` (main.ts), and a `data-add-instrument` button (index.html). Nothing in the serializer: the type is just a string in the JSON, so save/load/share/undo all ride along.
...
- **Share link**: `encodeToHash` (async) strips `backgroundImage` (URLs can't carry MB-scale data) **and all row/chair/instrument ids** (runtime identity only — incompressible UUID noise), deflates the JSON with the native `CompressionStream('deflate-raw')`, and base64**url**s the bytes under a `#2.` prefix (a symphony chart is ~900 chars vs ~24k under the old scheme). `decodeFromHash` (async) dispatches on the prefix — legacy v1 links (`btoa(encodeURIComponent(json))`, no prefix; '.' can't appear in btoa output) still decode — and regenerates fresh ids via `restoreIds`. Browsers without `CompressionStream` (pre-2023) fall back to *encoding* v1, which decodes everywhere. Returns `{ hash, strippedBackground }`; when stripped, the share-URL display appends a one-line warning telling the user to use Save JSON for full fidelity. The round-trip, legacy fallback, id regeneration and size bound are locked by `tests/serializer.test.ts`.
